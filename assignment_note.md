...
1. DTO 파싱 (`naver_stock_dtos.dart`)
...
**골든 테스트 일부 fail**
  | No. | 테스트 항목 (Test Description) | 결과 | 실패 사유 (Error Details) | 픽셀 차이 (Diff) | 관련 파일 |
  | :---: | :--- | :---: | :--- | :---: | :--- |
...
기존 단순 `Container`를 `Container(shadow) > ClipRRect > BackdropFilter > Container(background/border)` 3계층 구조로 재구성했습니다. 하트 + 체크 합성은 20×20 `SizedBox` 안에 `Stack`으로 구현했습니다.
